...
// Allow HR roles to see any ID, but restrict regular employees to their own data
...
try {
    // ✅ FIX 3: Robust Query with Security Check
    // If user is not HR/Admin, we add a check to the WHERE clause
    $is_hr = in_array($_SESSION['role_name'] ?? '', ['Admin', 'HR Officer', 'HR Staff']);
    ...
    // Security: Regular employees can only fetch their own leave
    if (!$is_hr) {
        $query .= " AND l.employee_id = ?";
        $stmt = $pdo->prepare($query);
        $stmt->execute([$application_id, $_SESSION['employee_id']]);
    } else {
        $stmt = $pdo->prepare($query);
    ...
    if (!$leave) {
        ...
        error_log("Unauthorized or failed leave access attempt for ID: $application_id by User: " . $_SESSION['employee_id']);
        ...
        <main class="main-content">
            ...
            <div class="dashboard-wrapper">
                ...
                <div class="bottom-actions">
                    <a href="approve-leave.php" class="back-btn">
