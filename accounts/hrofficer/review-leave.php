...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    try {
        // ✅ FIX 3: Get Fresh Officer Info (Name & Position)
        ...
        $officer_name = ($officer_info['first_name'] ?? 'Authorized') . ' ' . ($officer_info['last_name'] ?? 'Officer');
        ...
        $success_msg = "Application updated to " . $new_status . " successfully.";
        header("Refresh: 2; url=pending-leave.php");
    } catch (PDOException $e) {
        error_log("Officer Review Submit Error: " . $e->getMessage());
        $error_msg = "Database Error: Could not process the decision.";
    }
}
...
try {
    ...
    error_log("Officer Review Load Error: " . $e->getMessage());
    die("Database Error: Failed to load application data.");
}
?>
