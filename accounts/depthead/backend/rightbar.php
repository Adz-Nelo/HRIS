...
// CONFIGURATION
// ===========================
$default_profile_image = '/HRIS/assets/images/default_user.png';
$today = date('m-d');
...
// 1. Fetch Logged-in User's Department
$user_dept_id = null;
if ($logged_in_id) {
    $stmtDept = $pdo->prepare("SELECT department_id FROM employee WHERE employee_id = ?");
    $stmtDept->execute([$logged_in_id]);
    $user_dept_id = $stmtDept->fetchColumn();
}

// 2. Fetch Birthdays (Global or Dept-based? Usually global, but keeping global for now)
$birthdays = [];
try {
    $stmt = $pdo->prepare("
...
// 3. Fetch Active Employees IN THE SAME DEPARTMENT
$activeEmployees = [];
try {
    // Only proceed if we have a department ID
    if ($user_dept_id) {
        $stmt = $pdo->prepare("
            ...
            LIMIT 20
        ");
        $stmt->execute([$user_dept_id, $logged_in_id]);
...
<style>
    ...
    .role-badge { font-size: 9px; color: white; font-weight: 700; padding: 1px 6px; border-radius: 10px; text-transform: uppercase; width: fit-content; margin-bottom: 2px; }
</style>
...
<div class="right-sidebar" id="right-sidebar">
    <div class="sponsored-section">
        ...
        <a href="https://web.facebook.com/hrmsbacolod" target="_blank" class="sponsor-item" style="text-decoration:none; display:flex; align-items:center; gap:12px;">
            <img src="assets/images/sponsors.png" alt="HRMSBacolod" style="width:100px; border-radius:8px;">
    ...
    <div class="rightbar-header" style="display:flex; justify-content:space-between; align-items:center;">
        <span class="section-label">Contacts (My Dept)</span>
    ...
    <div class="employee-list">
        <?php if(!empty($activeEmployees)): ?>
            ...
            <?php endforeach; ?>
        <?php else: ?>
            <span style="font-size:11px; color:gray; padding-left:8px;">No other members in your department.</span>
        <?php endif; ?>
    </div>
</div>